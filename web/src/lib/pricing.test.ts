...
describe("estimateTotal", () => {
  ...
  it("multiplies rate by the matching quantity for each unit kind", () => {
    ...
    expect(estimateTotal({ price: 150, price_unit: "per performer" }, { performers: 4 })).toBe(
      600,
    );
  });
...
describe("quantityPhrase", () => {
  it("pluralizes based on the actual count", () => {
    ...
    expect(quantityPhrase("performer", { performers: 1 })).toBe("1 performer");
    expect(quantityPhrase("performer", { performers: 4 })).toBe("4 performers");
  });
